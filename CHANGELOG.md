...
### Changed
- **CI is green again** - `build-server` had been failing since at least 2026-06-29 because the workflow omitted `-tags sqlite_fts5`, which `mattn/go-sqlite3` needs for FTS5; every test touching `memory_messages_fts` died with "no such module: fts5". The tag is now passed to build, test, vet, staticcheck, gosec, govulncheck and golangci-lint, matching the Makefile. With staticcheck fixed the lint job also reached `golangci-lint` for the first time (a failing step skips the rest of its job), surfacing 37 unchecked error returns that are now handled.
- **Release images follow the repo** - the docker job pushed to a hardcoded `ghcr.io/comma-compliance/arc-relay`, the upstream org, so every release since the fork moved failed with `permission_denied`. The image is now derived from `${GITHUB_REPOSITORY}`, lowercased for GHCR.

### Security
- **`arc-sync skill install` accepted backslash path traversal on Windows** - `safeRelPath` checked only forward-slash traversal (`"../"`, `"/../"`), but its caller passes `filepath.Clean` output, which uses backslash separators on Windows. `..\escape.txt` matched none of the checks, so a crafted skill archive could write outside the destination directory when installed on Windows. Backslash is now folded to `/` before the traversal checks on every platform rather than only where `filepath` would: a guard whose verdict depends on `GOOS` is the wrong shape, and tar's only defined separator is `/`, so a backslash in an entry name is anomalous regardless of who extracts it. CI had been reporting this the whole time as a Windows-only failure of `TestExtractTarGz_RejectsTraversal`.
- **Upstream git URLs were unvalidated** - `PUT /api/skills/{slug}/upstream` is gated on the `skills:write` capability, which is deliberately grantable to non-admin API keys, and the checker cron passes whatever is stored straight to `git clone`. git's `ext::` transport executes an arbitrary command, and the `--` separator does not defend against it (`--` stops a URL being read as a flag; `ext::` is a scheme). Not exploitable as shipped — current git refuses `ext::` by default and `GIT_CONFIG_GLOBAL=/dev/null` blocked re-enabling it — but the protection was entirely inherited from a git default rather than owned by this code.
  - `store.ValidateGitURL` rejects `ext::`, any `transport::` helper, and a leading `-`; `gitEnv` pins `GIT_ALLOW_PROTOCOL` so the transport set is explicit.
  - Enforced inside `store.UpsertUpstream` rather than at the handler, because three paths write that row (JSON API, dashboard form, and an `X-Upstream` header on version upload). Same placement as `ValidateSlug` in `CreateSkill`.
  - `file://` and bare local paths remain allowed: they only reach local git repositories, which in the relay container means other skills' caches, already readable by anyone who can set an upstream.
